export default {
  common: {
    learnMore: 'En savoir plus',
    dog: 'Chien',
    cat: 'Chat',
    horse: 'Cheval',
    other: 'Autre',
    petCloning: 'Petcloning.eu',
  },
  cookies: {
    title: 'Nous utilisons des cookies',
    message:
      "Nous utilisons certains packages d'analyse standard pour comprendre le comportement général des utilisateurs, afin de savoir comment améliorer notre contenu. Cela implique l'utilisation de cookies. Êtes-vous d'accord ?",
    accept: 'Accepter',
    decline: 'Refuser',
  },
  seo: {
    home: {
      title: "Clonage d'animaux de compagnie - Accueil",
      description:
        'Tout ce que vous devez savoir sur le clonage de votre animal de compagnie.',
    },
    science: {
      title: "Clonage d'animaux de compagnie - Science",
      description:
        "Découvrez le processus complexe du clonage d'animaux de compagnie par transfert nucléaire de cellules somatiques (TNCS) et apprenez les étapes pour créer un nouvel animal avec la même ADN.",
    },
    petCloningGuide: {
      title:
        "Clonage d'animaux de compagnie - Guide de clonage d'animaux de compagnie",
      description:
        "Planifiez l'avenir ou gérez l'urgence du décès de votre animal de compagnie avec notre guide. Suivez les procédures correctes de préservation des cellules pour maintenir l'option de clonage ouverte avec un taux de réussite élevé.",
    },
    qA: {
      title: "Clonage d'animaux de compagnie - Questions et réponses",
      description:
        "Explorez une section complète de questions et réponses pour trouver des réponses à vos questions sur le clonage d'animaux de compagnie, le clonage reproductif et la science qui le sous-tend.",
    },
    cloningNews: {
      title: "Clonage d'animaux de compagnie - Actualités",
      description:
        "Obtenez les dernières mises à jour et actualités sur le clonage d'animaux de compagnie, les avancées dans le domaine et les percées dans la technologie de clonage reproductif.",
    },
    contactUs: {
      title: "Clonage d'animaux de compagnie - Contactez-nous",
      description:
        "Contactez-nous pour toute question ou demande d'informations supplémentaires sur le clonage d'animaux de compagnie. Nous sommes là pour fournir les détails dont vous avez besoin.",
    },
    aboutUs: {
      title: "Clonage d'animaux de compagnie - À propos de nous",
    },
  },
  validations: {
    requiredFields: 'Veuillez remplir tous les champs obligatoires',
    phoneFormat: 'Format de numéro de téléphone invalide',
    emailFormat: "Format d'email invalide",
    unsavedChanges:
      "Vous n'avez pas enregistré vos modifications. Êtes-vous sûr de vouloir quitter la page?",
  },
  navigation: {
    home: 'Accueil',
    science: 'Science',
    petCloningGuide: 'Guide du clonage',
    qA: 'Q&R',
    cloningNews: 'Nouvelles',
    contactUs: 'Contact',
    aboutUs: 'À propos de nous',
  },
  home: {
    cover: {
      title:
        'Tout ce que vous devez savoir sur le clonage de votre animal de compagnie',
    },
    section1: {
      title: `Bienvenue dans la possibilité de cloner des animaux de compagnie - une réalisation scientifique qui permet aux propriétaires d'envisager un avenir avec une partie de l'héritage de leur animal actuel.`,
      subtitle: `En essence, le clonage d'animaux de compagnie consiste à créer un nouvel animal avec la même composition génétique que l'original. C'est réel, accessible et réalisé en tenant compte de la santé et des détails.
      L'idée d'avoir le même animal peut sembler être un rêve, mais c'est une option disponible aujourd'hui. 
      
      Les coûts du clonage d'animaux de compagnie par des entreprises du monde entier reflètent son processus sophistiqué:`,
      subtitle_1: `Sur cette plateforme, nous fournissons des informations sur le clonage d'animaux de compagnie et la préservation des cellules, de son fonctionnement à ce qu'il implique financièrement et émotionnellement.         
      
      Si vous êtes curieux ou envisagez sérieusement de cloner votre animal de compagnie, vous trouverez ici des conseils directs et informatifs sur les étapes et les décisions impliquées dans la réalisation du clonage.`,
      prices: {
        dog: '50 000 € pour les chiens',
        cat: '50 000 € pour les chats',
        horse: '85 000 € pour les chevaux',
      },
    },
    section2: {
      title: 'Pourquoi cloner votre animal de compagnie?',
      subtitle:
        "Choisir de cloner un animal de compagnie est une décision personnelle et souvent émotionnelle. Cela reflète le désir de maintenir un lien physique au-delà de la durée de vie d'un animal de compagnie, associé à une appréciation des avancées en science qui rendent cela possible. Voici quelques raisons pour lesquelles les propriétaires d'animaux envisagent le clonage.",
      point1: {
        title: 'Prolonger le lien',
        subtitle:
          "Pour beaucoup, les animaux de compagnie sont plus que des animaux, ce sont des membres de la famille. Le clonage permet aux propriétaires de conserver une continuité génétique de leurs animaux de compagnie bien-aimés, même si la personnalité et les particularités de l'animal original sont uniques à chaque individu et à son expérience de vie.",
      },
      point2: {
        title: 'Caractéristiques valorisées',
        subtitle:
          'Certains animaux de compagnie possèdent des caractéristiques exceptionnelles, que ce soit leur lignée héréditaire, leurs qualités de race rares ou leurs compétences primées. Le clonage permet la préservation de ces traits génétiques précieux, notamment pour les chevaux de course primés, les chiens de spectacle ou les races rares.',
      },
      point3: {
        title: 'Progrès en science',
        subtitle:
          "La technologie derrière le clonage d'animaux de compagnie est un témoignage des progrès en génétique. C'est un domaine qui continue de croître et de fasciner, fournissant des informations sur la préservation et la reproduction génétiques.",
      },
    },
    section3: {
      title:
        "Étapes du clonage d'animaux de compagnie et préservation des cellules",
      subtitle:
        "Si vous envisagez de cloner votre animal de compagnie, ou si vous souhaitez conserver ses cellules par précaution au cas où vous voudriez le cloner à l'avenir, cela implique une série d'étapes scientifiques garantissant la création d'un nouvel animal avec la même composition génétique que votre compagnon original. Voici un aperçu simplifié de la manière dont le processus se déroule généralement.",
      step1: {
        title: "Collecte d'échantillon de tissu",
        subtitle:
          "Cela commence par la collecte d'un échantillon de tissu de votre animal. Il s'agit généralement d'un échantillon de peau, prélevé sous la supervision d'un vétérinaire, pour garantir que les cellules sont viables pour le clonage.",
      },
      step2: {
        title: 'Culture cellulaire',
        subtitle:
          'Les cellules collectées sont ensuite envoyées à une installation de culture cellulaire, où elles sont cultivées pour se développer et se multiplier, créant une ligne stable du matériel génétique de votre animal. Les cellules sont ensuite soit stockées indéfiniment pour la préservation des cellules, soit directement envoyées à une installation de clonage.',
      },
      step3: {
        title: 'Préservation des cellules',
        subtitle:
          "Les cellules sont ensuite congelées dans de l'azote liquide pour la préservation des cellules ou pour être envoyées à une installation de clonage. Elles peuvent rester congelées indéfiniment sans dommage.",
      },
      step4: {
        title: "Donneuse d'œufs et transfert",
        subtitle:
          "L'étape suivante consiste à obtenir un œuf d'un animal donneur. Le matériel génétique de cet œuf est remplacé par l'ADN des cellules cultivées de votre animal.",
      },
      step5: {
        title: 'Développement embryonnaire',
        subtitle:
          "Une fois le matériel génétique transféré, l'œuf commence à se développer en un embryon dans un environnement de laboratoire contrôlé, simulant le développement naturel.",
      },
      step6: {
        title: 'Mère porteuse et naissance',
        subtitle:
          "L'embryon est ensuite implanté dans une mère porteuse, où il se développera jusqu'à la naissance. Cette étape est cruciale et est effectuée avec une surveillance attentive pour garantir la santé et la sécurité de la mère porteuse et du clone en développement.",
      },
      step7: {
        title: 'Ramener votre clone à la maison',
        subtitle:
          'Une fois que le clone est né et sevré, des arrangements sont pris pour que le nouvel animal soit transporté chez vous.',
      },
    },
    section4: {
      text: `Bien que le processus soit scientifiquement complexe, l'objectif est simple: offrir aux propriétaires d'animaux de compagnie la possibilité de prolonger le lien qu'ils ont avec leurs animaux. 

Le clonage ne remplace pas l'animal que vous avez perdu, mais offre une façon unique de faire perdurer une partie d'eux dans le futur avec vous. 
      
Chaque étape est réalisée avec précision, soin et respect pour les vies impliquées dans ce voyage remarquable.`,
    },
  },
  science: {
    section1: {
      title: "La science derrière le clonage d'animaux de compagnie",
      subtitle:
        "Le clonage d'animaux de compagnie est une forme de clonage reproductif qui consiste à créer un nouvel animal avec le même ADN qu'un animal existant ou décédé. Le processus scientifique utilisé s'appelle le transfert nucléaire de cellules somatiques (TNCS). Voici comment cela fonctionne, expliqué en étapes compréhensibles.",
      step1: {
        title: 'Collecte et culture de cellules somatiques',
        subtitle:
          "Il commence par la culture de cellules somatiques vivantes, qui sont n'importe quelle cellule du corps sauf les cellules reproductrices (spermatozoïdes et ovules). Dans le clonage d'animaux de compagnie, les cellules utilisées sont généralement des fibroblastes prélevés à partir d'un fragment de peau et contiennent l'information génétique complète de l'animal de compagnie.",
      },
      step2: {
        title: 'Préservation des cellules',
        subtitle:
          "Si le clonage ne doit pas se produire immédiatement, les cellules sont stockées dans de l'azote liquide (-196°C) et peuvent rester stockées pendant des décennies si nécessaire. Pour commencer le processus de clonage, ces cellules congelées doivent d'abord être remises en culture.",
      },
      step3: {
        title: 'Transfert nucléaire',
        subtitle:
          "Le noyau, qui contient l'ADN, est retiré de la cellule somatique. Une cellule d'œuf est prélevée chez un animal donneur et son noyau est retiré pour faire place au noyau de la cellule somatique. Cette cellule d'œuf contient maintenant l'ADN de l'animal de compagnie, devenant essentiellement une copie génétique.",
      },
      step4: {
        title: 'Stimulation électrique',
        subtitle:
          "L'œuf reconstruit, avec le nouveau noyau, est stimulé par un courant électrique ou des produits chimiques, ce qui l'encourage à se diviser et à devenir un embryon, tout comme s'il y avait eu une fécondation naturelle.",
      },
      step5: {
        title: 'Développement embryonnaire',
        subtitle:
          "Cet embryon est ensuite autorisé à se développer en blastocyste - un stade précoce du développement adapté à l'implantation dans l'utérus d'une mère porteuse. À ce stade, le processus de clonage est le même que pour le processus de fécondation in vitro.",
      },
      step6: {
        title: 'Implantation et grossesse',
        subtitle:
          "Le blastocyste développé est implanté dans l'utérus de la mère porteuse, où il continue de se développer. Si l'implantation réussit, la mère porteuse devient enceinte.",
      },
      step7: {
        title: 'Naissance',
        subtitle:
          "La mère porteuse mène la grossesse à terme et donne naissance à l'animal de compagnie cloné.",
      },
      conclusion:
        "Chaque animal cloné est un duplicata génétique de l'animal original, ce qui signifie qu'ils partagent le même ADN. Cependant, il est important de noter que bien que l'animal cloné, tout comme les jumeaux identiques, ressemblera beaucoup à l'original, il peut ne pas agir de manière identique, car l'environnement et l'éducation façonnent également le comportement et la personnalité.",
    },
    section2: {
      title: "Histoire du clonage d'animaux de compagnie",
      subtitle:
        "L'histoire du clonage d'animaux de compagnie ne peut être racontée sans mentionner la brebis emblématique nommée Dolly. Née en 1996 à l'Institut Roslin en Écosse, Dolly a été le premier mammifère à être cloné à partir d'une cellule somatique adulte grâce à la technologie du transfert nucléaire. Sa création réussie a démontré que les cellules spécialisées pouvaient être reprogrammées pour créer un organisme entier, une découverte qui a fait avancer la science et ouvert la voie au clonage d'animaux. Après Dolly, la technologie a fait d'énormes progrès.",
      step1: {
        title: 'Étapes initiales',
        subtitle: `En 1998, le clonage de souris a été réalisé, prouvant que la technique pouvait être étendue au-delà des moutons.

        En 2000, la naissance de CC ("CopyCat"), le premier chat cloné, a marqué un moment décisif pour les propriétaires d'animaux de compagnie dans le monde entier.`,
      },
      step2: {
        title: 'Élargissement de la portée',
        subtitle: `Chiens et chevaux : Les années suivantes ont vu le clonage d'animaux plus complexes en termes de clonage. Les chiens, qui représentaient un défi significatif en raison de leur cycle de reproduction unique, ont été clonés avec succès en 2005. Les chevaux, eux aussi, ont été clonés, avec des implications pour la préservation des génétiques équines d'élite.
        
        Bétail et espèces en danger : Les efforts se sont également tournés vers le bétail et les espèces en danger, prouvant la valeur du clonage en agriculture et en conservation. Des animaux tels que des porcs, des vaches et même des espèces en danger comme le gaur ont été clonés, ouvrant des discussions sur la biodiversité et la prévention de l'extinction.`,
      },
      step3: {
        title: 'Affinage des techniques',
        subtitle:
          "Les progrès dans le TNCS et les améliorations dans la science génétique ont conduit à des processus de clonage plus efficaces et réussis. L'ajustement de ces protocoles a abouti à des taux de réussite plus élevés et à une réduction de la probabilité des anomalies observées dans les premiers clones.",
      },
      step4: {
        title: 'Clonage commercial',
        subtitle:
          "Dans les années 2010, le clonage commercial d'animaux de compagnie est devenu accessible au public, bien que cela ait un coût élevé. Avec ce service, les propriétaires d'animaux de compagnie pouvaient chercher à cloner un animal de compagnie bien-aimé, que ce soit pour des raisons sentimentales, pour préserver des traits exceptionnels ou à des fins compétitives.",
      },
      step5: {
        title: 'État actuel',
        subtitle:
          "Aujourd'hui, le clonage n'est plus seulement une possibilité théorique ou une expérience scientifique ; c'est un service disponible pour ceux qui le choisissent pour leurs animaux de compagnie. Alors que les débats éthiques et les cadres réglementaires continuent d'évoluer, le fait demeure que la technologie du clonage est passée d'une merveille scientifique à un choix personnel pour les propriétaires d'animaux de compagnie du monde entier. La technologie et les prix ont évolué pour rendre le clonage plus accessible et continueront de le faire à l'avenir.",
      },
    },
    section3: {
      title: "Progrès futur du clonage d'animaux de compagnie",
      subtitle:
        "La frontière du clonage d'animaux de compagnie s'avance vers un avenir passionnant et largement inconnu. Les progrès à venir dépendront des avancées scientifiques, des considérations éthiques et de la demande publique.",
      step1: {
        title: 'Avancées en génétique et reproduction',
        subtitle:
          "On s'attend à ce que les progrès futurs continuent à améliorer l'efficacité et la fiabilité du clonage. Les avancées de la technologie d'édition génique CRISPR et de la recherche sur les cellules souches pourraient conduire à des améliorations dans la production des clones et les résultats de santé des animaux clonés. Cela pourrait rendre le processus plus accessible et réduire les coûts au fil du temps, ouvrant potentiellement la voie à davantage de propriétaires d'animaux de compagnie envisageant le clonage comme une option viable.",
      },
      step2: {
        title: 'Préservation génétique améliorée',
        subtitle:
          "À mesure que la technologie de clonage mûrit, les efforts de conservation des espèces en voie de disparition à l'aide des techniques de clonage deviendront plus sophistiqués. Cela peut inclure des efforts pour réintroduire la diversité génétique dans des populations en déclin, contribuant à leur récupération et durabilité.",
      },
      step3: {
        title: 'Personnalisation et prévention des maladies',
        subtitle:
          "Avec l'avancement de la compréhension de la base génétique des maladies, il existe un potentiel pour que les animaux de compagnie clonés soient moins susceptibles aux conditions héréditaires, en effectuant un dépistage et éventuellement une modification génétique avant le clonage, assurant ainsi une durée de vie plus saine pour la progéniture résultante.",
      },
      step4: {
        title: 'Développement éthique et réglementaire',
        subtitle:
          "L'avenir verra probablement un environnement réglementaire plus défini à mesure que le clonage devient plus courant. Les discussions éthiques entourant les pratiques de clonage peuvent influencer la réglementation et aboutir à des pratiques mondiales standardisées garantissant le bien-être des animaux clonés.",
      },
      step5: {
        title: 'Engagement public et éducation',
        subtitle:
          "Accroître la connaissance du public sur le clonage grâce à l'éducation jouera un rôle critique dans son développement. À mesure que la compréhension augmente, la conversation sur les implications morales et pratiques du clonage se développera, façonnant ainsi l'orientation de l'industrie.",
      },
      step6: {
        title:
          'Clonage pour la recherche sur le vieillissement et les maladies',
        subtitle:
          "Le clonage peut servir d'outil de recherche significatif pour l'étude du vieillissement et des maladies chroniques. En produisant des animaux génétiquement identiques, les scientifiques peuvent étudier de manière plus précise la progression des maladies et l'efficacité des traitements, ce qui peut avoir des implications profondes tant pour la médecine humaine que vétérinaire.",
      },
    },
  },
  petCloningGuide: {
    section1: {
      title: "Guide du clonage d'animaux de compagnie",
      subtitle:
        "Que vous planifiiez à l'avance le clonage de votre animal de compagnie en bonne santé dans le futur ou que vous soyez confronté à la situation urgente du décès d'un animal de compagnie, comprendre et suivre les procédures correctes de préservation des cellules est essentiel. Ce guide vise à vous assurer que vous prenez toutes les mesures nécessaires pour maintenir l'option du clonage ouverte avec les taux de réussite les plus élevés possibles. En cas de situation urgente (décès récent de l'animal de compagnie),",
      subtitle_1: 'pour obtenir des conseils plus spécifiques.',
    },
    section2: {
      title: 'Pour les animaux de compagnie décédés',
      point1: {
        title: 'Agissez rapidement et gardez la fraîcheur',
        subtitle:
          'Après le décès de votre animal de compagnie, ne congelez pas son corps ni les tissus collectés. Placez le corps ou les tissus dans un environnement frais (températures de réfrigérateur de 2 à 4 °C) pour ralentir la décomposition cellulaire.',
      },
      point2: {
        title: "Demandez l'assistance vétérinaire",
        subtitle:
          "Un vétérinaire peut garantir que l'échantillon est prélevé correctement et stocké dans une solution physiologique stérile, que l'on peut se procurer dans la plupart des pharmacies.",
      },
      point3: {
        title: 'La collection stérile est cruciale',
        subtitle:
          "Lors de la collecte d'un échantillon de peau, maintenir la stérilité est aussi essentiel que la promptitude. Les échantillons contaminés peuvent ne pas être viables pour le clonage. Si vous ne pouvez pas trouver un vétérinaire pour effectuer la collecte, utilisez des instruments stérilisés et des gants avec de l'éthanol.",
      },
    },
    section3: {
      title:
        "Pour la préservation des cellules d'un animal de compagnie vivant",
      point1: {
        title: 'Conditions idéales',
        subtitle:
          "Si vous décidez de cloner votre animal de compagnie alors qu'il est encore en vie, organiser une biopsie cutanée stérile sous la supervision d'un vétérinaire est le meilleur scénario. Cela minimise le stress sur les cellules et maximise leur viabilité pour le clonage.",
      },
      point2: {
        title: 'Stockage et transport sécurisés',
        subtitle:
          "Que vous anticipiez un clonage futur ou agissiez après une perte, l'échantillon collecté doit être conservé à une température constante de réfrigérateur de 2 à 4 °C, à la fois pendant le stockage et lors de l'expédition vers l'installation de clonage ou de culture cellulaire.",
      },
      point3: {
        title: 'Le temps est essentiel',
        subtitle:
          "Plus rapidement un échantillon est sécurisé et livré au laboratoire de clonage, meilleures sont les chances de clonage réussi, que ce soit à partir d'un animal de compagnie vivant ou d'un animal récemment décédé.",
      },
    },
    section4: {
      title: 'Envoi de votre échantillon',
      point1: {
        title: 'Guidance professionnelle',
        subtitle: "Avant l'échantillonnage,",
        subtitle_1: "pour des informations sur les conditions d'expédition.",
      },
      point2: {
        title: 'Expédition sécurisée et fraîche',
        subtitle:
          "Utilisez un contenant réfrigérant pour le transport et assurez-vous qu'il reste scellé et à la température fraîche et stable requise.",
      },
    },
    section5: {
      text: "En suivant ces procédures, vous augmentez considérablement la probabilité d'un clonage réussi.\n\nLes mesures de clonage proactives pour les animaux de compagnie vivants donnent généralement un taux de réussite plus élevé en raison des conditions optimales des cellules et de l'environnement contrôlé lors de la collecte.\n\nN'oubliez pas, la qualité de l'échantillon et les soins pris lors de ces étapes initiales sont fondamentaux pour le parcours de création d'une continuation génétique de votre animal de compagnie précieux.",
    },
  },
  qA: {
    title: 'Questions & Réponses',
    q1: {
      title: "Qu'est-ce que le clonage d'animaux de compagnie?",
      subtitle:
        "R: Le clonage d'animaux de compagnie est le processus de création d'une copie génétiquement identique d'un animal. Cela est réalisé en prélevant un échantillon de peau de l'animal d'origine (le donneur), en créant des cellules cultivées, puis en transférant le matériel génétique de ces cellules dans un ovule dont le noyau a été retiré.",
    },
    q2: {
      title: "Combien coûte le clonage d'un animal de compagnie?",
      subtitle:
        "R: Les coûts de clonage peuvent être importants, reflétant la complexité scientifique impliquée. Actuellement, cloner un chat ou un chien peut coûter environ 50 000 €, tandis qu'un cheval peut coûter jusqu'à 85 000 €.",
    },
    q3: {
      title: "Quel est l'intérêt de la conservation des cellules?",
      subtitle:
        "R: La conservation des cellules est comme une assurance qui vous permettra de cloner votre animal de compagnie à l'avenir si vous le choisissez. Décider de cloner un animal de compagnie juste après son décès peut être une situation stressante et le risque de faire une erreur ou d'être dans une situation rendant le clonage impossible est assez élevé. Avec la conservation des cellules, vous pouvez vous préparer en toute sécurité à un clonage potentiel et, s'il y a un problème, il est toujours possible de le refaire.",
    },
    q4: {
      title:
        'Que dois-je faire si mon animal vient de mourir et que je envisage le clonage?',
      subtitle:
        "R: Il est crucial d'éviter que le corps ou tout tissu ne gèle et de le conserver plutôt au réfrigérateur. Contactez immédiatement un vétérinaire pour prélever un échantillon de tissu dans un milieu stérile, puis prenez contact avec nous pour d'autres instructions sur le stockage et le transport.",
    },
    q5: {
      title: 'Puis-je cloner un animal décédé depuis longtemps?',
      subtitle:
        "R: Le clonage réussi nécessite un échantillon génétique de haute qualité. Une fois qu'un animal est décédé depuis longtemps, la dégradation de l'ADN rend le clonage moins probable. Il est préférable de commencer le processus dès que possible après le décès. Généralement, si l'animal est mort depuis une semaine ou plus et n'a pas été conservé à 4 degrés, il ne peut pas être cloné. S'il est conservé à 4 degrés juste après la mort, un animal peut être cloné jusqu'à 3 semaines après le décès.",
    },
    q6: {
      title:
        'Pourquoi ne devrais-je pas congeler le corps de mon animal si je veux le cloner?',
      subtitle:
        "R: La congélation peut endommager la structure cellulaire et l'ADN à l'intérieur, ce qui est nécessaire pour le clonage. Le maintien de l'échantillon au réfrigérateur préserve l'intégrité des cellules et de l'ADN, augmentant les chances de clonage réussi.",
    },
    q7: {
      title: "Puis-je cloner mon animal à partir d'un échantillon de poil?",
      subtitle:
        "R: Non, il n'existe aucune méthode de clonage à partir d'un poil. L'ADN seul n'est pas suffisant pour le clonage car celui-ci nécessite un transfert de noyau, donc des cellules vivantes (généralement de la peau) sont nécessaires.",
    },
    q8: {
      title:
        "Mon animal cloné peut-il avoir la même personnalité que mon animal d'origine?",
      subtitle:
        "R: Bien qu'un animal cloné aura la même composition génétique que votre animal d'origine, les traits de personnalité sont influencés à la fois par la génétique et l'environnement. Par conséquent, l'animal cloné peut avoir des prédispositions similaires mais développera sa propre personnalité unique grâce à ses expériences individuelles.",
    },
    q9: {
      title: "Le clonage d'animaux de compagnie est-il sûr?",
      subtitle:
        'R: Les processus de clonage se sont considérablement améliorés au fil du temps et sont réalisés avec des normes élevées de soins vétérinaires. Cependant, comme tout processus biologique, il existe des risques inhérents et des taux de réussite variables.',
    },
    q10: {
      title:
        "Quel est le taux de réussite pour le clonage d'animaux de compagnie?",
      subtitle:
        "R: Les taux de réussite du clonage varient en fonction de l'état et de la manipulation de l'échantillon génétique, des techniques de clonage utilisées et de l'animal individuel. La qualité de l'ADN et le démarrage rapide du processus sont des facteurs clés.",
    },
    q11: {
      title:
        "Comment puis-je transporter l'échantillon de mon animal vers une installation de clonage?",
      subtitle:
        "R: Si vous souhaitez cloner votre animal, remplissez le formulaire de contact et nous vous donnerons tous les détails sur la manière d'envoyer l'échantillon de peau de votre animal à l'installation de culture cellulaire.",
    },
    q12: {
      title: "Le clonage d'animaux de compagnie est-il légal en Europe?",
      subtitle:
        "R: Dans l'Union européenne, seul le clonage d'animaux destinés à la consommation, tels que le bétail, est réglementé. Le clonage d'animaux de compagnie n'est pas défini comme illégal par l'Union européenne. Chaque pays européen peut avoir une réglementation différente sur le sujet, mais pour la plupart d'entre eux, il n'y a tout simplement aucune réglementation. Le clonage d'un animal de compagnie à l'étranger et son retour dans votre pays ne posera aucun problème légal, car seul le clonage lui-même est réglementé, et non le fait de posséder un animal cloné.",
    },
  },
  contactUs: {
    title: 'Demander des informations',
    name: 'Prénom et Nom',
    phoneMobile: 'Téléphone / Mobile',
    email: 'E-mail',
    country: 'État',
    message: 'Message',
    petName: "Nom de l'animal",
    animalType: "Type d'animal",
    cloningInfo: "Je souhaite plus d'informations sur le clonage",
    preservationInfo:
      "Je souhaite plus d'informations sur la préservation des cellules",
    replySubject: 'Petcloning.eu - Confirmation de demande',
    replyText: `Merci beaucoup d'avoir tendu la main !

Cet e-mail de réponse automatique sert simplement à vous informer que nous avons reçu votre demande et qu'un membre de notre équipe vous répondra dans les plus brefs délais.
    
Pendant les heures de bureau, cela prend généralement quelques heures.
    
En attendant, vous pouvez trouver ci-dessous des informations générales sur le clonage et la préservation des cellules.
---------------------
Cordialement,
Petcloning.eu`,
    buttonText: 'Envoyer la demande',
    requestSuccess: 'Votre demande a été envoyée!',
    requestError:
      "Une erreur s'est produite lors du traitement de votre demande. Veuillez réessayer!",
  },
  footer: {
    petCloning: 'Petcloning.eu',
    contactUs: 'Contactez-nous',
    copyright: '© 2023, Tous droits réservés',
    language: 'Langue',
  },
  placeholders: {
    name: 'Jean Dupont',
    phoneMobile: '(123) 456-7890',
    countryCode: '+33',
    email: 'jeandupont@mail.com',
    country: 'France',
    petName: 'Max',
    message: 'Écrivez votre message ici...',
  },
  aboutUs: {
    ourValues: {
      title: 'Nos valeurs',
      text: `Nous avons créé petcloning.eu pour permettre aux gens en Europe d'avoir accès à la technologie de clonage pour leur animal de compagnie bien-aimé. Jusqu'à présent, le clonage était disponible en Asie et en Amérique mais était très difficile d'accès pour les Européens. Nous voulons fournir une solution sûre, transparente et éthique pour les clients qui souhaitent cloner leurs chiens, chats ou chevaux. 
      Nous souhaitons également faire partie d'une meilleure technologie de clonage, plus efficace et respectueuse des animaux et nous consacrons notre expertise à essayer d'augmenter l'efficacité du clonage.`,
    },
    whoWeAre: {
      title: 'Qui sommes-nous',
      text: "L'activité de Petcloning.eu a commencé en 2022 à Split, en Croatie. Nous avons maintenant aussi un laboratoire à Zagreb où la plupart de la culture cellulaire est réalisée, tandis que le laboratoire à Split est dédié à la recherche. Nous travaillons en collaboration avec les installations de clonage les plus expérimentées au monde qui ont déjà cloné des milliers d'animaux de compagnie du monde entier.",
      francois: {
        name: 'Francois-Xavier Pellay, PhD',
        title: 'Co-Fondateur et CSO',
        description: `Le Dr Pellay est un scientifique et entrepreneur français avec plus de 15 ans d'expérience dans l'industrie de la longévité et une passion de toute une vie pour la science et le vieillissement. 
        
        Il a co-fondé Petcloning.eu pour relier son expertise en biologie du vieillissement et le besoin de solutions améliorées dans la technologie de clonage. Son intérêt pour le domaine est également lié au fait que le clonage est en quelque sorte la solution de rajeunissement la plus efficace jamais trouvée pour rajeunir les cellules vieillies au point de créer des animaux jeunes et en bonne santé.`,
      },
      ronny: {
        name: 'Ronny Haesen',
        title: 'Co-Fondateur et COO',
        description: `M. Haesen est un entrepreneur belge avec de l'expérience dans différents domaines d'activité, y compris dans l'industrie de la biotechnologie depuis 2017. 
        
        Au cours du processus de facilitation de la culture cellulaire pour les installations de clonage en Asie et aux États-Unis, nous avons rapidement constaté qu'il y avait peu d'informations ou d'aide disponibles pour les Européens intéressés à en savoir plus sur le clonage. C'était la principale raison pour laquelle, avec son partenaire commercial de longue date, le Dr Pellay, il a fondé Petcloning.eu.`,
      },
    },
    inTheNews: {
      title: 'Petcloning.eu dans les actualités',
      article1: {
        date: '22/02/2024',
        country: 'France',
        page: 'usbeketrica.com',
      },
      article2: {
        date: '08/03/2024',
        country: 'Croatie',
        page: 'slobodnadalmacija.hr',
      },
      article3: {
        date: '08/03/2024',
        country: 'Croatie',
        page: 'net.hr',
      },
    },
  },
};
